...
├── workflows/reusable-refresh-automation-hub-token.yml   called only by the skeleton repo itself
...
- **Step tests** in `tests/`. Each decision-making `run:` step is extracted
  ...
  Nox workflow's refusal of matrix sessions marked default and of a sanity
  ignore file not carried forward to the newest ansible-core, the
